%This is for generating simulation data when true coefficients are smoothly varying.

%This program is directly modified from "MST_unequal".  
...
phi=0.1;%range parameter for the covariates
which_pattern=1;
...
% "smooth_cluster" for smoothly varying coefficients
...
if strcmp(scenario,'constant_cluster')
   ...
   for zz=1:length(unique_z_true)
     temp_index=find(group_index_save_intercept==unique_z_true(zz));
     if (unique_z_true(zz)==1)
		 Intercept_smooth(temp_index)=((lon(temp_index))+(lat(temp_index))).^1.7;
     end
	 if (unique_z_true(zz)==3)
		Intercept_smooth(temp_index)=((lon(temp_index))+(lat(temp_index))).^1.7;
	 end
...
group_index_save=repmat(1,length(group_index_save),1);
group_index_save_intercept=repmat(2,length(group_index_save_intercept),1);
