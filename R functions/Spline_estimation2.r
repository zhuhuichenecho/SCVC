#This program is the same as "Spline_estimation", except return different variables.

BIC_TPS2=function(lambda2_S_seq,lambda2_I_seq,B,BB,E,Emp)
{
tt=1
BIC=vector(length=0)
AIC=vector(length=0)
...
beta_svc_S=IB%*%spline_coef[1:L]
beta_svc_I=IB%*%spline_coef[(L+1):pL]
...
return(list(beta_svc_S=beta_svc_S,beta_svc_I=beta_svc_I,AIC=AIC,BIC=BIC,lambda2_S=lambd2_S,lambda2_I=lambd2_I))
